...
# [3] 그룹 절 과 집계함수 , 그룹 ( ~~ 별  ~~ 끼리 )
-- mid 기준으로 그룹하여 총 구매 수량을 구해
select mid , sum( bamount ) from buy GROUP BY mid;  -- 1) mid 기준으로 그룹하여 총 구매 수량 ( bamount )
select mid , sum(bamount * bprice ) 총구매금액 
    from buy group by mid; -- 2) mid 기준으로 총 구매 금액 ( 수량 * 가격)
select count(*) from buy -- 3) mid (회원)별 판매 횟수
select count(*) , mid from buy group by mid;

# [4] 그룹절에 조건절, where 그룹*전* 조건  [vs] having 그룹*후* 조건 
select * from buy where bamount >3;
select mid , sum(bamount) 총구매수량 
    from buy group by mid having 총구매수량 > 5;
select mid , sum(bamount) 총구매수량 from buy where 총구매수량 > 5 GROUP BY mid;  -- from이 우선실행되고 where조건을 확인하기 때문에 
-- 별칭인 총구매수량이 뭔지 몰라서 오류가 생긴다.  ( where 절을 먼저 처리 )

-- [순서] select 필드명 from 테이블명 where 조건절 group by 그룹필드

# [5]  order by 정렬 , desc 내림차순 , asc 오름차순
select * from member order by mdebut;
select * from member order by mdebut DESC;
# [*] 다중정렬이란 ? 첫번째 정렬 후 첫번째 정렬 필드 기준으로 중복이 존재할 경우 중복끼리 2차 정렬
-- 1차 정렬 지역(maddr) 먼저 정렬하고 지역 필드내 * 동일한 값 끼리* 2차 정렬 ( mdebut ) 한다. 
...
select * from member limit 5 , 5 ; -- 5번 부터 5개 (5) / 활용 1페이지 ( 0부터 5개) 
