...
use mydb0807;  -- ctrl + shift + enter // 전체 실행
create Table test ( -- create table 테이블명 (속성명 타입 제약조건 ...)
    번호 int AUTO_INCREMENT,
    constraint PRIMARY key( 번호 ),
    이름 VARCHAR(30) not NULL UNIQUE,
    개수 int DEFAULT 2
);
-- DML : 조작어 , 1) insert 레코드삽입 2) select 레코드조회 3) update 레코드 수정 4) delete 레코드 삭제
--  레코드란 ? 테이블내 행/가로 단위 
# [1] insert : insert into 테이블명 ( 속성명, 속성명 2) values ( 값1, 값2 )
...
select * from test;
insert into test (이름) VALUES ("신동엽");
select * from test;

-- insert into test ( 이름 ) VALUES ("유재석");
insert into test VALUES(4,"하하" , 30);
...
SELECT * from test where 이름 = "유재석"; -- 이름 속성명 값이 유재석이면 조회같다.
select * from test where 개수 >= 5; -- 개수 속성명이 값이 5이상이면 조회

# [3] update : update 테이블명 set 속성명 = 새로운값 , 속성명 = 새로운값 where 조건)
update test set 개수 = 10; -- 조건이 없으므로 테이블내 '개수' 속성들의 값 *모두 수정됨*
update test set 개수 = 30 where 이름 = "유재석";
update test set 개수 = 40, 이름 = "강호동2" where 번호 = 2;
--
# [4] delect : delete : delete from 테이블명 WHERE 조건
...
select mid 회원아이디 from member 회원테이블; -- as를 생략하고 띄어쓰기 로 별칭이 가능 함. 

# [2] distinct , 조회 결과의 속성값 중복 제거 
select DISTINCT maddr from member;

# [3] 산술연산자 : +더하기 -빼기 /나누기 *곱하기    div몫 mod 나머지 
...
select * from member where mphone1 = null; -- 데이터베이스에서 null 처리방법
select * from member where mphone1 is null; -- =null [x] / is null[0]을 써야됨. 
-- 문자열 패턴 : 속성명 like "문자패턴" ,  %: 모든 문자대응  , _: 개수만큼 문자대응  , 
