...
public class Exam3 {
    public static void main(String[] args) {
        /*
            ...
            main 함수란 ? JAVA / C / PYTHON / JS(브라우저) (html) / JAVA 내 단일 스레드 환경 main 메소드가 제공된다.
            ...
            주요 메소드 
                ...
                2) start( ) : 구현된 run ( ) 메소드 실행하는 메소드  , 주의할점 ] 스레드 시작점
                (확장에 대비하려면 인터페이스에서 구현하는것이 좋음 )

            * 활용처 : 1) 톰켓(웹서버 / SPRING) : 다수의 유저(클라이언트)가 1개의 서버(PC)에 요청시 병렬 처리 제공
        ...
        */
        ...
        Runnable runnable = new Runnable() { 
            ...
            @Override public void run(){
                for( int i = 1; i <= 5 ; i++){
                    System.out.println("새로운 첫번째 스레드");
                    try{ Thread.sleep(1000); }catch(Exception e){};
                }
            }
        }; //  익명구현체 구현 끝
        ...
        thread2.start();  // 총 스레드의 갯수는 // MAIN / TASK1 / TASK2    -> 총 3개 실행됨.
        

        // [4] 멀티스레드 방법 3: 상속
        작업스레드3 thread3 = new 작업스레드3();
        thread3.start();        // MAIN / TASK1 / TASK2 / TASK3   = 병렬처리 ( 처리 순서 보장은 없음. )
